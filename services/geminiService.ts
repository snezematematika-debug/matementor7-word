...
export const generateQuizQuestions = async (topic: string, grade: string): Promise<{questions: QuizQuestion[], rubric: string}> => {
  try {
    ...
    const prompt = `
      ...
      Рубриката треба да содржи:
      1. Точните одговори за сите прашања.
      2. Распределба на поени (на пр. "Задача 1: 5 поени. 2 поени за постапка, 3 за точен резултат").
      3. Критериуми за делумни поени.
